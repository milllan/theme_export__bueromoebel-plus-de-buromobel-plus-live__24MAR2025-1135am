...
var createCartPromise = () => {
  return new Promise(async (resolve) => {
    if (window.BOLD && BOLD.common){
      resolve(await BOLD.common.cartDoctor.fix(await ( await fetch(`${Shopify.routes.root}cart.js`)).json()));
    }
    else {
...
var createCartPromise = () => {
  return new Promise(async (resolve) => {
    try {
      // Check if BOLD and BOLD.common.cartDoctor exist
      if (window.BOLD && BOLD.common && BOLD.common.cartDoctor && typeof BOLD.common.cartDoctor.fix === 'function') {
        // Fetch the cart data
        const cartData = await (await fetch(`${Shopify.routes.root}cart.js`)).json();
        // Apply the cartDoctor fix if available
        const fixedCartData = await BOLD.common.cartDoctor.fix(cartData);
        resolve(fixedCartData);
      } else {
        // If cartDoctor doesn't exist, resolve with the cart data as is
        const cartData = await (await fetch(`${Shopify.routes.root}cart.js`)).json();
        resolve(cartData);
      }
    } catch (error) {
      console.error('Error in createCartPromise:', error);
      resolve(null); // Resolve with null or handle the error appropriately
    }
  });
};
